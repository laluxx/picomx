...
static void init_animation(session_t *ps, struct managed_win *w) {
	...
	if (ps->o.wintype_option[w->window_type].animation != OPEN_WINDOW_ANIMATION_INVALID && !w->wm_mask) {
	...
	{
		auto prop_desktops = x_get_prop(ps->c, ps->root, ps->atoms->a_NET_CURRENT_DESKTOP, 1L, XCB_ATOM_CARDINAL, 32);
		if (prop_desktops.nitems) {
			ps->prev_desktop_num = ps->root_desktop_num;  // Store the value of the current desktop number as the new previous desktop number
			ps->root_desktop_num = (int)*prop_desktops.c32;
			auto prop_num_desktops = x_get_prop(ps->c, ps->root, ps->atoms->a_NET_NUMBER_OF_DESKTOPS, 1L, XCB_ATOM_CARDINAL, 32);
			if (prop_num_desktops.nitems) {
				ps->inversing = false;
				ps->root_max_desktop_num = (int)*prop_num_desktops.c32;
				if (ps->prev_desktop_num == ps->root_desktop_num) {
				ps->root_desktop_switch_direction = 0;
				if (ps->root_desktop_switch_direction == 0) {animation = ps->o.animation_for_unmap_window;}
				} else if (ps->prev_desktop_num < ps->root_desktop_num) {ps->root_desktop_switch_direction = 1;} 
				else if (ps->prev_desktop_num > ps->root_desktop_num) {ps->root_desktop_switch_direction = -1;}
				//else if(ps->prev_desktop_num == (0||ps->root_max_desktop_num) && ps->root_desktop_num != (ps->root_max_desktop_num||0))
				//{ps->root_desktop_switch_direction = !ps->root_desktop_switch_direction;ps->inversing = true;}
			}
		}
		if(ps->root_desktop_switch_direction<0)
		{
			animation = ps->o.animation_for_prev_tag;
			if (ps->o.enable_fading_prev_tag){w->state = WSTATE_FADING;}
			w->animation_is_tag |= animation >= OPEN_WINDOW_ANIMATION_ZOOM ? ANIM_FAST : ANIM_SLOW;
			w->animation_is_tag |= ANIM_FAST;
			w->wm_mask = ANIM_UNMAP;
		}
		if(ps->root_desktop_switch_direction>0)
		{
			animation = ps->o.animation_for_next_tag;
			if (ps->o.enable_fading_next_tag){w->state = WSTATE_FADING;}
			w->animation_is_tag |= animation >= OPEN_WINDOW_ANIMATION_ZOOM ? ANIM_FAST : ANIM_SLOW;	
			w->animation_is_tag |= ANIM_FAST;
			w->wm_mask = ANIM_UNMAP;
		}
		if (w->wm_mask & ANIM_UNMAP) {
	...
	case OPEN_WINDOW_ANIMATION_SLIDE_UP: // Slide up the image, without changing its location
		if (w->wm_mask & ANIM_SPECIAL_MINIMIZE) {
	...
	case OPEN_WINDOW_ANIMATION_SLIDE_LEFT:  // Slide left the image, without changing its location
		if (w->wm_mask & ANIM_SPECIAL_MINIMIZE) {
		if(ps->inversing){*anim_x = w->pending_g.x;*anim_y = w->pending_g.y + w->pending_g.height * 0.5;}
		else{*anim_x = w->pending_g.x + w->pending_g.width;*anim_y = w->pending_g.y + w->pending_g.height * 0.5;}	
		} else {
		if(!ps->inversing){*anim_x = w->pending_g.x + w->pending_g.width;*anim_y = w->pending_g.y + w->pending_g.height * 0.5;}
		else{*anim_x = w->pending_g.x;*anim_y = w->pending_g.y + w->pending_g.height * 0.5;}	
		}
		*anim_w = 0;
...
void win_process_update_flags(session_t *ps, struct managed_win *w) {
	...
	if (win_check_flags_any(w, WIN_FLAGS_SIZE_STALE | WIN_FLAGS_POSITION_STALE)) {
		...
		if (win_should_animate(ps, w)) {
			...
			if (!was_visible || w->wm_mask) {
				// Set window-open animation
				init_animation(ps, w);
				if (!(w->wm_mask & ANIM_SPECIAL_MINIMIZE)||!(w->wm_mask & ANIM_UNMAP)) {
					w->animation_dest_center_x = w->pending_g.x + w->pending_g.width * 0.5;
			...
			w->animation_inv_og_distance = 1.0 / sqrt(x_dist * x_dist + y_dist * y_dist + w_dist * w_dist + h_dist * h_dist);
